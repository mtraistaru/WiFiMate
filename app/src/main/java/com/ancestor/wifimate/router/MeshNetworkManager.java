...
public class MeshNetworkManager {
    ...
    public static String getIPForClient(AllEncompasingP2PClient c) {
        ...
        if (self.getGroupOwnerMac().equals(self.getMac())) {
            if (!self.getGroupOwnerMac().equals(c.getGroupOwnerMac()) && go.getIsDirectLink()) {
                // not the same group owner, but we have the group owner as a
                // direct link
                return c.getIp();
            } else if (go != null && !self.getGroupOwnerMac().equals(c.getGroupOwnerMac()) && !go.getIsDirectLink()) {
